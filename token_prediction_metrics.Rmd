...
plot_metrics <- function(model_name, is_ft, metrics_group) {
  chain <- params$chain
  ...
  for (predict_region in predict_regions) {
    ...
    metrics <- json_data[names(json_data) %in% c(metrics_group)]
  ...
  metrics_order <- c("f1-score", "precision", "recall")

  p <- ggplot(metrics_all, aes(x = factor(region, levels = regions_order),
...
for (model in models) {
  print(model)
  for (ft in c(FALSE, TRUE)) {
    for (metrics_group in c("0", "1", "weighted avg", "macro avg")) {
      p <- plot_metrics(model, ft, metrics_group)
      print(p)
    }
  }
}
