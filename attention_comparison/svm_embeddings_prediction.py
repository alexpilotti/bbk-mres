...
def compute_prediction(data_path, embeddings_path, output_path,
    ...
    print(f"Class size: {collections.Counter(np.sort(y)).most_common()}")
    print(f"In total, {len(y)} sequences from {len(np.unique(y_groups))} "
          "donors/studies.")
    print(f"Class size: {collections.Counter(np.sort(y)).most_common()}")
    ...
    for train_index, test_index in outer_cv_w_groups:
        ...
        print(f"% positive train: {np.mean(y_train)}, "
